...
const CreateChannel = (props: Props) => {
  ...
  const findUsers = async () => {
    ...
    try {
      const response = await client.queryUsers({ id: { $autocomplete: inputText } });
  ...
  const findUsersDebounce = _debounce(findUsers, 100, { trailing: true });
  ...
  const createChannel = async () => {
    ...
    if (!selectedUsersIds.length || !client.userID) {
      return;
    }

    const conversation = client.channel('messaging', { members: [...selectedUsersIds, client.userID] });
